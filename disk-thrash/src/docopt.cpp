...
namespace {
	...
	std::vector<T*> flat_filter(docopt::Pattern& pattern) {
		std::vector<docopt::Pattern*> flattened = pattern.flat([](docopt::Pattern const* p) -> bool {
			return dynamic_cast<T const*>(p) != nullptr;
		});

		// now, we're guaranteed to have T*'s, so just use static_cast
		std::vector<T*> ret;
		std::transform(flattened.begin(), flattened.end(), std::back_inserter(ret), [](docopt::Pattern* p) {
