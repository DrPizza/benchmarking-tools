...
int main(int, char*[]) {
	...
	__cpuidex(cpu.data(), leaf::extended_limit, 0x0);
	if (cpu[register_t::eax] >= leaf::brand_string_2) {
		union {
			std::array<char, 48> brand;
			std::array<std::array<int32_t, 4>, 3> registers;
		} data;
		__cpuidex(data.registers[0].data(), leaf::brand_string_0, 0x0);
		__cpuidex(data.registers[1].data(), leaf::brand_string_1, 0x0);
		__cpuidex(data.registers[2].data(), leaf::brand_string_2, 0x0);
		std::cout << data.brand.data() << std::endl;
	}
	if (cpu[register_t::eax] < leaf::advanced_power_management) {
		std::cout << "I can't perform timing on this chip" << std::endl;
		return -1;
	}

	std::array<int32_t, 4> registers = { 0 };
	__cpuidex(registers.data(), leaf::advanced_power_management, 0x0);
	if (0ui32 == (registers[register_t::edx] & (1ui32 << 8ui32))) {
		std::cout << "I can't perform timing on this chip" << std::endl;
		return -1;
	}
